...
SESSION="${SESSION:hashcat}"
DICT="${DICT:dicts/rockyou.txt}"
...
# check Linux
	...
	if [ $(uname -m) == 'x86_64' ]; then
		HASHCAT="./hashcat-5.1.0/hashcat64.bin"
	else
		HASHCAT="./hashcat-5.1.0/hashcat32.bin"
	fi
# check Windows
...
"$HASHCAT" -m "$HASH_TYPE" -a 0 "$HASH_FILE" --session="$SESSION" "$DICT" --potfile-path "$POT_FILE"

# DICTIONARY ATTACK WITH RULES------------------------------------------------------------
# now lets move on to a rule based attack, d3ad0ne.rule is a great one to start with
"$HASHCAT" -m "$HASH_TYPE" -a 0 "$HASH_FILE" --session="$SESSION" "$DICT" -r hashcat-5.1.0/rules/d3ad0ne.rule --potfile-path "$POT_FILE"

# rockyou is pretty good, and not too slow
"$HASHCAT" -m "$HASH_TYPE" -a 0 "$HASH_FILE" --session="$SESSION" "$DICT" -r hashcat-5.1.0/rules/rockyou-30000.rule --potfile-path "$POT_FILE"


# MEDIUM
# dive is a great rule file, but it takes a bit longer to run, so we will run it after d3ad0ne and rockyou
"$HASHCAT" -m "$HASH_TYPE" -a 0 "$HASH_FILE" --session="$SESSION" "$DICT" -r hashcat-5.1.0/rules/dive.rule --potfile-path "$POT_FILE"

# HEAVY
# MASK ATTACK (BRUTE-FORCE)---------------------------------------------------------------
"$HASHCAT" -m "$HASH_TYPE" -a 3 "$HASH_FILE" --session="$SESSION" hashcat-5.1.0/masks/rockyou-1-60.hcmask --potfile-path "$POT_FILE"
